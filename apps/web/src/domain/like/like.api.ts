...
export class LikeApi {
  ...
  static findManyByMemeId(
    memeId: string,
    queryOptions?: ApiHelper.QueryOptions<Like>,
  ): Promise<Like[]> {
    const buildOptions = ApiHelper.buildQueryOptions(queryOptions)

    return HttpService.api.get(`/v1/memes/meme/${memeId}/likes${buildOptions}`)
  }

  static createOneByMemeId(
    memeId: string,
    values: Partial<Like>,
  ): Promise<Like> {
    return HttpService.api.post(`/v1/memes/meme/${memeId}/likes`, values)
  }
}
